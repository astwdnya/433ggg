...
impl AfPacketBackend {
    pub fn open(upstreams: &[SocketAddr]) -> Result<Self, SnifferError> {
        ...
        // ccmni interfaces (MediaTek mobile data) use
        // a proprietary link type [519] with no Ethernet header.
        // Use SOCK_DGRAM + ETH_P_IP for raw IP mode on these interfaces.
        let is_rawip = ifname.starts_with("rmnet") || ifname.starts_with("ccmni");

        let (sock_type, proto) = if is_rawip {
            (libc::SOCK_DGRAM, (libc::ETH_P_IP as u16).to_be() as i32)
        } else {
            (libc::SOCK_RAW, (libc::ETH_P_ALL as u16).to_be() as i32)
        };

        let fd = unsafe {
            libc::socket(libc::AF_PACKET, sock_type, proto)
        };
        if fd < 0 {
        ...
        // BPF filter assumes Ethernet framing; skip it for raw IP interfaces
        if !is_rawip {
            attach_bpf_filter(fd, upstreams)?;
        }

        Ok(AfPacketBackend { fd, ifindex, is_rawip })
    }
}
...
impl RawBackend for AfPacketBackend {
    ...
    fn send_frame(&mut self, frame: &[u8]) -> Result<(), SnifferError> {
        ...
        // Ethernet interfaces need destination MAC in sll_addr;
        // raw IP interfaces (rmnet/ccmni) have no link-layer addressing.
        if !self.is_rawip {
            sll.sll_halen = 6;
